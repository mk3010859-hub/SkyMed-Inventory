// ============================================================
// DATABASE MODULE - TiDB Cloud Compatible
// ============================================================
...
// ============================================================
// MAIN DATABASE CLASS - TiDB Compatible
// ============================================================
class SkyMedDB {
    constructor() {
        this.db = null;
    ...
    loadCacheFromStorage() {
        try {
            const cached = localStorage.getItem('skymed_cache_v2');
            if (cached) {
    ...
    saveCacheToStorage() {
        try {
            localStorage.setItem('skymed_cache_v2', JSON.stringify(this.dataCache));
    ...
    // ============================================================
    // CRUD OPERATIONS (Hybrid: TiDB + IndexedDB)
    // ============================================================
    ...
    async add(store, data) {
        try {
            ...
            // Try to save to TiDB via backend
            try {
                await this.apiCall(store, {
                    method: 'PUT',
                    body: JSON.stringify(data)
                });
                console.log('✅ Saved to TiDB:', store, data.id);
            } catch (e) {
                console.warn('⚠️ TiDB save failed, saved to local cache only:', e);
            }
    ...
    async delete(store, id) {
        return new Promise((resolve, reject) => {
            try {
                ...
                // Try to delete from TiDB via backend
                try {
                    await this.apiCall(`${store}/${id}`, {
                        method: 'DELETE'
                    });
                } catch (e) {
                    console.warn('⚠️ TiDB delete failed:', e);
                }
    ...
    async init() {
        try {
            ...
            console.log('Database initialized (TiDB Compatible Mode)');
...
console.log('Database module loaded! (TiDB Compatible Mode)');
